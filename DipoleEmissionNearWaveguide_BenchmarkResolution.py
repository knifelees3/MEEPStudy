# This is to do a resolution benchmarks
import meep as mp
...
pg_mat=np.zeros(num_swep)
neff_mat=np.zeros(num_swep)

for l in range(num_swep):
    resolution=resolution_mat[l]
    ...
    pt_mat[l]=ptotal
    pg_mat[l]=pmode_1+pmode_2
    sim.reset_meep()


print('Simulations Complete !')
effic_mat=pg_mat/pt_mat

fig1=plt.figure(figsize=(6.4*2,4.8))
plt.subplot(121)
plt.plot(resolution_mat,pt_mat,'rs-',label='total power')
plt.plot(resolution_mat,pg_mat,'ro-',label='guided power')
plt.legend(loc='best')
plt.title('Total power and guided power')
plt.xlabel('resolution')
plt.ylabel('power (un-normalized)')
plt.subplot(122)
plt.plot(resolution_mat,effic_mat,'rs-',label='total power')
plt.legend(loc='best')
plt.title('The coupling efficiency')
plt.xlabel('resolution')
plt.ylabel('The coupling efficiency')
plt.savefig('benchmarkresolution_dipolenearguide.png')

# run 
# mpirun -np 20  python DipoleEmissionNearWaveguide.py > output.log